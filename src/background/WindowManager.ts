...
import { upsertObjectKeyValue } from '@/modules/util/Objects';
...
function enableCORSHeaderPatch(window: BrowserWindow) {
    ...
    window.webContents.session.webRequest.onBeforeSendHeaders((details, callback) => {
        ...
        upsertObjectKeyValue(requestHeaders, 'Access-Control-Allow-Origin', ['*']);
        callback({ requestHeaders });
    });

    window.webContents.session.webRequest.onHeadersReceived((details, callback) => {
        const { responseHeaders } = details;
        upsertObjectKeyValue(responseHeaders, 'Access-Control-Allow-Origin', ['*']);
        upsertObjectKeyValue(responseHeaders, 'Access-Control-Allow-Headers', ['*']);
        callback({
            responseHeaders
        });
    });
